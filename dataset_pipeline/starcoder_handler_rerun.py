...
def run_script(id, rerun_list, main_tests_file, output_file, logger_file, total_logger_dir):
    start_time = time.time()
    start_time_str = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(start_time))
    print(f"Running the script {output_file} now.")
    total_logger1 = os.path.join(total_logger_dir, "total_logger.txt")
    total_logger2 = os.pat h.join(total_logger_dir, "total_logger2.txt")
    with open(total_logger1, 'a') as total_log_file:
        ...
        try:
            ...
            rerun_list.remove(id)
            with open(total_logger2, 'a') as timeout_log_file:
                timeout_log_file.write(f"\nThe rerun_list: {rerun_list}\n")

# solve original dataset
def extract_and_save_script_tests_rerun(dataset_path, output_dir):
    jsons = read_jsonl_file(dataset_path)
    rerun_list = get_rerun_list(f"{output_dir}/total_logger2.txt")
    print(f"{output_dir} Rerun list: {rerun_list}")
    for problem_dict in jsons:
        id = problem_dict['id']
        if str(id) not in rerun_list:
            continue
        ...
        run_script(str(id), rerun_list, f"{output_dir}/{id}/main_tests_{id}.py", f"{output_dir}/{id}/PDBscript_main_tests_{id}.py", f"{output_dir}/{id}/logger_PDBscript_{id}.txt", output_dir)
...
if __name__ == "__main__":
    ...
    extract_and_save_script_tests_rerun(path_file,path_directory)
#python -u "/Users/heartflowyu/Desktop/新加卷F/code_llm/starcoder_handler_command.py" 